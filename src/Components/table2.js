      ...
      <div>
        <div className="table-responsive top1">
          ...
          <Table className="table">
            <tbody>
              {this.state.leavereports &&
                this.state.leavereports.map(report => {
                  if (report && report.hasOwnProperty("leaveReport")) {
                    return (
                      <tr className="head-tab">
                        <td className="leave_report_col1">
                          {report.leaveReport && report.leaveReport.dates}{" "}
                          <br />
                        ...
                        <td className="leave_report_col2">
                          {report.leaveReport && report.leaveReport.days}
                        </td>
                        <td
                          className={report.leaveReport.approvalstatus.toLowerCase()}
                        >
                          {report.leaveReport &&
                            report.leaveReport.approvalstatus}
                        </td>
                        {report.leaveReport.approvalstatus.toLowerCase() ===
                        "pending" ? (
